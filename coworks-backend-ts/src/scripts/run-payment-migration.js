...
const migration = require('../migrations/add-payment-status-column');

async function runMigration() {
  // Create a Sequelize instance using your database connection string
  const sequelize = new Sequelize(process.env.DATABASE_URL || 'postgres://postgres:password@localhost:5432/excel_coworks', {
    dialect: 'postgres',
    logging: console.log,
    dialectOptions: {
  ...
  try {
    ...
    // Run the migration
    console.log('Running migration to add payment_status column...');
