...
export async function PUT(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!valid || !decoded || decoded.role !== 'admin') {
      return NextResponse.json<ApiResponse<null>>(
        {
          success: false,
    ...
    try {
      ...
      // Update seating type with interval and rate settings
      await seatingType.update({
        ...body,
        // Set default values for interval flags if not provided
        is_hourly: body.is_hourly ?? seatingType.is_hourly,
        is_daily: body.is_daily ?? seatingType.is_daily,
        is_weekly: body.is_weekly ?? seatingType.is_weekly,
        is_monthly: body.is_monthly ?? seatingType.is_monthly,
        // Set rates to current values if not provided
        hourly_rate: body.hourly_rate ?? seatingType.hourly_rate,
        daily_rate: body.daily_rate ?? seatingType.daily_rate,
        weekly_rate: body.weekly_rate ?? seatingType.weekly_rate,
        monthly_rate: body.monthly_rate ?? seatingType.monthly_rate
      });
