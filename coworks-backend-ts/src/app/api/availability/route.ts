...
export async function GET(request: NextRequest) {
  try {
    ...
    if (seatId) {
      ...
      if (!seatingTypeId && !seatingTypeCode) {
        return NextResponse.json(
          { 
            error: "Missing seating type parameter", 
            details: "Either seating_type_id or seating_type_code is required when querying by branch_id"
          },
          { status: 400 }
        );
      }
