...
// Handle interval change
  ...
  // Handle rate change
  const handleRateChange = (rateType: string, value: number) => {
    setRates(prev => ({
      ...prev,
      [rateType]: value
    }));
  ...
  const handleFormSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (currentSeatingType.id) {
        ...
        const seatingTypeData = {
          ...currentSeatingType,
          ...intervalSettings,
          ...rates
        };
        response = await axios.put(`/api/seating-types/${currentSeatingType.id}`, seatingTypeData, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
        ...
        response = await axios.post('/api/seating-types', seatingTypeData, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Dialog open={formOpen} onClose={handleFormClose} maxWidth="md" fullWidth>
        <DialogTitle>
          {currentSeatingType?.id ? 'Edit' : 'Add'} Seating Type
        </DialogTitle>
        <DialogContent>
          <Grid container spacing={2}>
            <Grid item xs={12}>
              <Typography variant="h6" gutterBottom>Booking Intervals</Typography>
              <Grid container spacing={2}>
                <Grid item xs={12} sm={6} md={3}>
                  <FormControlLabel
                    control={<Checkbox
                      checked={intervalSettings.is_hourly}
                      onChange={(e) => handleIntervalChange('is_hourly', e.target.checked)}
                    />}
                    label="Hourly"
                  />
                  {intervalSettings.is_hourly && (
                    <TextField
                      label="Hourly Rate"
                      type="number"
                      value={rates.hourly_rate}
                      onChange={(e) => handleRateChange('hourly_rate', parseFloat(e.target.value))}
                      fullWidth
                      margin="dense"
                    />
                  )}
                ...
                <Grid item xs={12} sm={6} md={3}>
                  <FormControlLabel
                    control={<Checkbox
                      checked={intervalSettings.is_daily}
                      onChange={(e) => handleIntervalChange('is_daily', e.target.checked)}
                    />}
                    label="Daily"
                  />
                  {intervalSettings.is_daily && (
                    <TextField
                      label="Daily Rate"
                      type="number"
                      value={rates.daily_rate}
                      onChange={(e) => handleRateChange('daily_rate', parseFloat(e.target.value))}
                      fullWidth
                      margin="dense"
                    />
                  )}
                ...
                <Grid item xs={12} sm={6} md={3}>
                  <FormControlLabel
                    control={<Checkbox
                      checked={intervalSettings.is_weekly}
                      onChange={(e) => handleIntervalChange('is_weekly', e.target.checked)}
                    />}
                    label="Weekly"
                  />
                  {intervalSettings.is_weekly && (
                    <TextField
                      label="Weekly Rate"
                      type="number"
                      value={rates.weekly_rate}
                      onChange={(e) => handleRateChange('weekly_rate', parseFloat(e.target.value))}
                      fullWidth
                      margin="dense"
                    />
                  )}
                ...
                <Grid item xs={12} sm={6} md={3}>
                  <FormControlLabel
                    control={<Checkbox
                      checked={intervalSettings.is_monthly}
                      onChange={(e) => handleIntervalChange('is_monthly', e.target.checked)}
                    />}
                    label="Monthly"
                  />
                  {intervalSettings.is_monthly && (
                    <TextField
                      label="Monthly Rate"
                      type="number"
                      value={rates.monthly_rate}
                      onChange={(e) => handleRateChange('monthly_rate', parseFloat(e.target.value))}
                      fullWidth
                      margin="dense"
                    />
                  )}
